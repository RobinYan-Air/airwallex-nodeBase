...
const defaultFormatter = (info) => `${defaultTimeStamp()} [${info.level}] ${JSON.stringify(info.message)}`

const transTransportsFromObject = (obj) => {
...
for (const level of errorLevel) {
  Object.assign(Logger.prototype, {
    [level](str) {
      // add optional traceId
      this[FUNSEE_LOGGER][level](this.app.traceId ? `🐞 traceId: ${this.app.traceId} 🐞 🐞 ${str}` : str)
    }
  });
}
